...
export async function POST(req: NextRequest) {
  try {
    ...
    // If no API key configured yet, provide domain-aware fallback response
    if (!apiKey) {
      const lower = message.toLowerCase();
      let reply =
        "Hello! I am your StudyAbroad Vista AI Counsellor. I'm currently running in preview mode while the Gemini API key is being linked.";

      if (lower.includes("germany") || lower.includes("free")) {
        reply =
          "Germany offers 0 tuition fees at almost all public universities for both Bachelor's and Master's degrees! You only need a blocked account of approx. €11,904 (approx. ₹10.5 Lakhs) for living expenses, and you get an 18-month post-study work visa.";
      } else if (
        lower.includes("mbbs") ||
        ...
        reply =
          "For Indian medical aspirants, we recommend NMC & WHO compliant universities in Russia, Uzbekistan, Kazakhstan, Georgia, and the Philippines. Total packages range from ₹18 to ₹35 Lakhs including hostel and food, with English medium curriculum.";
      } else if (lower.includes("ausbildung")) {
        reply =
          "Germany's Ausbildung is a government-recognized 3-year dual vocational training program. There are zero tuition fees, and you receive a monthly stipend of €1,000–€1,400 while training in Healthcare, IT, or Engineering. You need German language proficiency (B1/B2 level).";
      } else if (
        lower.includes("usa") ||
        lower.includes("us") ||
        lower.includes("stem")
    ...
    return NextResponse.json(
      {
        ...
        fallback:
          "Our AI Counsellor is momentarily unavailable. Please select one of our curated guides or book a consultation.",
      },
      { status: 500 },
    );
  }
