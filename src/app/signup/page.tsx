...
import { Compass, GraduationCap, Briefcase, Building2, Mail, Lock, Phone, User, ArrowRight } from "lucide-react";
...
export default function SignupPage() {
  ...
  const { login } = useAuth();
  const [role, setRole] = useState<"student" | "buyer" | "university">("student");
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login(email.trim(), role as UserRole, fullName, data.user?.id);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col justify-between selection:bg-[#EA5C2B]/15">
        ...
        <div className="w-full max-w-md rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-[0_4px_24px_rgba(0,0,0,0.03)]">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-xs text-slate-500 leading-relaxed">
              Join 50,000+ Indian students, verified consultancies, and universities.
            </p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-3">
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                {role === "university" ? "Institutional Email" : role === "buyer" ? "Work Email" : "Email Address"}
              </label>
              <div className="relative rounded-lg border border-slate-200 bg-white transition focus-within:border-[#102C57] focus-within:ring-2 focus-within:ring-[#102C57]/10">
                <input
                  type="email"
                  ...
                  placeholder={role === "university" ? "admissions@university.edu" : role === "buyer" ? "director@consultancy.com" : "student@example.com"}
                  className="w-full rounded-lg py-2 px-3 text-xs text-slate-800 placeholder-slate-400 focus:outline-none"
                />
              </div>
            </div>

            {/* Conditional Org / University Field */}
            {role !== "student" && (
              <div>
                <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                  {role === "buyer" ? "Consultancy / Agency Name" : "University / College Name"}
                </label>
                <div className="relative rounded-lg border border-slate-200 bg-white transition focus-within:border-[#102C57] focus-within:ring-2 focus-within:ring-[#102C57]/10">
                  <input
                    type="text"
                    ...
                    placeholder={role === "buyer" ? "Apex Global Admissions" : "Technical University of Munich"}
                    className="w-full rounded-lg py-2 px-3 text-xs text-slate-800 placeholder-slate-400 focus:outline-none"
                  />
                </div>
              </div>
            ...
            {/* Phone Field */}
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">WhatsApp / Mobile Number</label>
              <div className="relative flex rounded-lg border border-slate-200 bg-white transition focus-within:border-[#102C57] focus-within:ring-2 focus-within:ring-[#102C57]/10">
                <span className="flex items-center pl-3 pr-2 text-xs font-semibold text-slate-500 border-r border-slate-100">+91</span>
                <input
                  type="tel"
                  maxLength={10}
                  value={phone}
            ...
            {/* Password Field */}
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 mb-1">Password (Min. 8 characters)</label>
              <div className="relative rounded-lg border border-slate-200 bg-white transition focus-within:border-[#102C57] focus-within:ring-2 focus-within:ring-[#102C57]/10">
                <input
                  type="password"
                  required
            ...
            <div className="space-y-2 pt-1">
              <label className="flex items-start gap-2 text-xs text-slate-600 cursor-pointer select-none">
                ...
                <span className="text-[11px] leading-tight text-slate-500">
                  I agree to the <span className="text-slate-800 font-medium underline">Terms</span> & <span className="text-slate-800 font-medium underline">Privacy Policy</span>, consenting to academic data processing under the <strong className="text-slate-700 font-semibold">DPDP Act 2023</strong>. <span className="text-rose-500 font-bold">*</span>
                </span>
              </label>
              ...
              <label className="flex items-start gap-2 text-xs text-slate-600 cursor-pointer select-none">
                ...
                />
                <span className="text-[11px] leading-tight text-slate-500">
                  Send scholarship deadlines, intake alerts, and visa updates via WhatsApp (Optional).
                </span>
              </label>
            </div>
            ...
            >
              {loading ? "Creating account..." : `Create ${role === "student" ? "Student" : role === "buyer" ? "Consultant" : "University"} Account →`}
            </button>
          </form>
      ...
      <footer className="border-t border-slate-200/80 bg-white py-3 text-center text-[11px] text-slate-400">
        © 2026 StudyAbroad Vista • Protected by 256-bit Encryption & Supabase Auth
      </footer>
    </div>
  );
}
