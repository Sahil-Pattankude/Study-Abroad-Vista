...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const exam = TEST_PREP_EXAMS.find((e) => e.slug.toLowerCase() === slug.toLowerCase());
...
export default async function TestPrepDetailPage({ params }: Props) {
  ...
  const relatedExams = TEST_PREP_EXAMS.filter((e) => e.id !== exam.id && (e.category === exam.category || (exam.category === "Graduate Admissions" && e.category === "English Proficiency")));
        ...
        <main className="flex-1 pb-16">
          ...
          <div className="border-b border-slate-200/80 bg-white py-2.5 px-4 sm:px-8">
            <div className="mx-auto flex max-w-7xl items-center gap-2 text-xs font-semibold text-slate-500">
              ...
              <Link href="/test-prep" className="hover:text-[#102C57] transition">Test Prep Hub</Link>
              <ChevronRight className="h-3.5 w-3.5 text-slate-400" />
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Exam Fee (INR)</span>
                  <p className="mt-1 text-base font-black text-[#EA5C2B]">{exam.feeINR}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Duration</span>
                  <p className="mt-1 text-sm font-bold text-white">{exam.duration}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Score Validity</span>
                  <p className="mt-1 text-sm font-bold text-white">{exam.validityYears === 99 ? "Lifetime" : `${exam.validityYears} Years`}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Score Scale</span>
                  <p className="mt-1 text-sm font-bold text-white">{exam.scoringScale}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Indian Target Cutoff</span>
                  <p className="mt-1 text-sm font-black text-emerald-400">{exam.targetCutoffIndianStudents}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xs">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Delivery Mode</span>
                  <p className="mt-1 text-xs font-bold text-white truncate" title={exam.formatMode}>{exam.formatMode.split(" ")[0]}</p>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 space-y-10">
                ...
                <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs">
                  ...
                  <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-slate-100 pt-6">
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Target Degrees & Programs</h3>
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {exam.targetPrograms.map((prog) => (
                          <span key={prog} className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">
                            {prog}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Conducting Body & Frequency</h3>
                      <p className="mt-2 text-xs font-bold text-slate-800">{exam.conductingBody}</p>
                ...
                <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-slate-400">{exam.sections.length} Sections</span>
                  </div>

                  <div className="mt-6 space-y-4">
                    {exam.sections.map((sec, idx) => (
                      <div key={idx} className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-5 hover:border-[#102C57]/30 transition">
                        ...
                        <div className="mt-3 text-xs text-slate-600">
                          <p><strong className="text-slate-800">Skills Tested:</strong> {sec.skillsTested}</p>
                          <div className="mt-2.5 rounded-xl bg-orange-50/80 border border-orange-200/50 p-3 text-slate-700">
                            <p className="text-[11px] font-bold text-[#EA5C2B] uppercase tracking-wider mb-0.5">Strategy for Indian Students:</p>
                ...
                <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Minimum eligibility thresholds vs. competitive scores for top-tier university admissions and visa approvals:
                  </p>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-50 text-slate-700">
                          ...
                          <th className="p-3 font-bold text-emerald-800">Competitive Target Score</th>
                          <th className="p-3 font-bold text-right">Destination Guide</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 text-slate-600">
                        {exam.scoreRequirementsByCountry.map((req, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/80 transition">
                            ...
                            <td className="p-3 font-medium text-slate-700">{req.minRequired}</td>
                            <td className="p-3 font-bold text-emerald-700">{req.competitiveScore}</td>
                ...
                <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Proven phased roadmap designed for working professionals and university students in India:
                  </p>

                  <div className="mt-6 space-y-4">
                    {exam.prepRoadmap.map((item, idx) => (
                      <div key={idx} className="relative flex gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5">
                        ...
                        <div>
                          ...
                          <p className="mt-2 text-xs leading-relaxed text-slate-600">{item.milestone}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="sticky top-24 space-y-6">
                  <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-md">
                    ...
                    <p className="mt-2 text-xs text-slate-500 leading-relaxed">
                      Share your target intake, destination, and current score to receive personalized university shortlisting and fee waiver tips.
                    </p>
                    ...
                    <ul className="space-y-3 text-xs">
                      {relatedExams.slice(0, 4).map((rel) => (
                        <li key={rel.id}>
                          ...
                          >
                            <div>
                              <p className="font-bold text-slate-900 group-hover:text-[#EA5C2B] transition">{rel.name}</p>
                              <p className="text-[10px] text-slate-400">{rel.feeINR} • {rel.category.split(" ")[0]}</p>
                            </div>
